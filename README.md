...
1. 进去【监控】页面，在【其他】-->【告警插件】页面进行上传即可
   ...
   ![image-20240411114214590](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411114214590.png)

   ![image-20240411114246138](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411114246138.png)
...
1. 登录manager，按照下图指引上传即可
   ...
   ![image-20240411114056545](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411114056545.png)

   ![](https://s3.yaourt.cn/img/windows/2024/04/16/202404161520820.png)

2. 在【告警规则查询】页面修改相应的规则，添加插件即可

   ![image-20240411114601967](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411114601967.png)
...
![image-20240411160219276](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411160219276.png)
...
​	![image-20240411112024971](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411112024971.png)
...
1. 使用编辑器打开脚本，根据2.1.3章节获取到的相关信息填入脚本，webhook_key参数仅用于当监控项中未配置key时则采用该参数进行推送，属于兜底策略
   ...
   ![image-20240417094117748](https://s3.yaourt.cn/img/windows/2024/04/17/202404170941889.png)
...
3. 监控告警推送引用插件相关参数配置
   ...
   **(可选)接收人手机号**：如果需要监控推送@指定相关人员，则在该项填入手机号即可
   ...
   ![image-20240417094256262](https://s3.yaourt.cn/img/windows/2024/04/17/202404170942353.png)
...
2. 打开插件引用并配置Webhook Key
   ...
   ![image-20240411154757360](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411154757360.png)

   ![image-20240411154835662](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411154835662.png)

3. 【告警规则查询】页面将需要监控的配置项设置推送，如有需要可配置指定接收人手机号

   ![image-20240411154923383](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411154923383.png)
...
![image-20240412091156081](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412091156081.png)
...
1. corpid
   ...
   ![image-20240411170851724](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411170851724.png)

2. secret

   在【管理后台】->【应用管理】->【应用】->【自建】，点进某个应用，即可看到，同agentid且配套使用

   ![image-20240411171059453](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411171059453.png)

   ![image-20240411171122161](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411171122161.png)
...
4. userid(用户id)
   ...
   ![image-20240411171349796](https://s3.yaourt.cn/img/windows/2024/04/11/image-20240411171349796.png)
...
1. 使用编辑器打开脚本，根据2.2.3章节获取到的相关信息填入脚本
   ...
   ![image-20240416161443577](https://s3.yaourt.cn/img/windows/2024/04/16/202404161614689.png)
...
3. 监控告警推送引用插件相关参数配置
   ...
   **PS：**俩个选项均不能留空，否则不允许发送
   ...
   ![image-20240416161903141](https://s3.yaourt.cn/img/windows/2024/04/16/202404161619262.png)
...
2. 根据2.2.3章节获取的相关参数填入到配置项
   ...
   ![image-20240416162221950](https://s3.yaourt.cn/img/windows/2024/04/16/202404161622083.png)
...
![image-20240412122112324](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412122112324.png)
...
​	![image-20240412103028965](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412103028965.png)

​	![image-20240412103107462](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412103107462.png)

3. 添加完成后在Webhook栏可以看到参数`access_token`，在安全设置栏可以配置IP地址以及加签秘钥

![image-20240416164552101](https://s3.yaourt.cn/img/windows/2024/04/16/202404161645223.png)
...
1. 使用编辑器打开脚本，根据3.1.3章节获取到的相关信息填入脚本，webhook_access_token以及webhook_secret参数仅用于当监控项中未配置时则采用该参数进行推送，属于兜底策略
   ...
   ![image-20240417094346930](https://s3.yaourt.cn/img/windows/2024/04/17/202404170943016.png)
...
3. 监控告警推送引用插件相关参数配置
   ...
   **(可选)接收人手机号, 用以@：**当需要推送的消息@群成员的话可以填入手机号
   ...
   ![image-20240417094506108](https://s3.yaourt.cn/img/windows/2024/04/17/202404170945199.png)
...
2. 根据3.1.3章节获取的相关参数填入到配置项
   ...
   **加签密钥(Secret), 可选**：安全设置中的加签秘钥，获取方式参考3.1.3章节，若客户现场不涉及无需填写
   ...
   ![image-20240416165522362](https://s3.yaourt.cn/img/windows/2024/04/16/202404161655513.png)
...
![image-20240416165906168](https://s3.yaourt.cn/img/windows/2024/04/16/202404161659300.png)
...
2. 在应用后台https://open-dev.dingtalk.com/fe/app#/corp/app，找到发布的应用，在【凭证与基础信息】栏找到相关参数信息
   ...
   ![image-20240416170733660](https://s3.yaourt.cn/img/windows/2024/04/16/202404161707795.png)
...
1. 使用编辑器打开脚本，根据3.2.3章节获取到的相关信息填入脚本
   ...
   ![image-20240416171237145](https://s3.yaourt.cn/img/windows/2024/04/16/202404161712278.png)

2. 监控告警推送引用插件相关参数配置

   **接收人手机号：**可从平台已有用户信息引入也可手动输入手机号

   ![image-20240416171314200](https://s3.yaourt.cn/img/windows/2024/04/16/202404161713319.png)
...
2. 根据3.2.3章节获取的相关参数填入到配置项
   ...
   ![image-20240416171503321](https://s3.yaourt.cn/img/windows/2024/04/16/202404161715474.png)
...
![image-20240412174537357](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412174537357.png)
### 4.1.2. 参数说明
...
1. 使用编辑器打开脚本，根据4.1.3章节获取到的相关信息填入脚本，webhook_token以及webhook_secret参数仅用于当监控项中未配置时则采用该参数进行推送，属于兜底策略
   ...
   ![image-20240417094631912](https://s3.yaourt.cn/img/windows/2024/04/17/202404170946001.png)
...
3. 监控告警推送引用插件相关参数配置
   ...
   **(可选)签名校验**：安全设置中的加签秘钥，获取方式参考4.1.3章节，若客户现场不涉及无需填写
   ...
   ![image-20240417094648929](https://s3.yaourt.cn/img/windows/2024/04/17/202404170946013.png)
...
![image-20240416174203886](https://s3.yaourt.cn/img/windows/2024/04/16/202404161742053.png)
...
![image-20240412185018465](https://s3.yaourt.cn/img/windows/2024/04/12/image-20240412185018465.png)
...
1. 使用编辑器打开脚本，根据3.2.3章节获取到的相关信息填入脚本
   ...
   ![image-20240416175614017](https://s3.yaourt.cn/img/windows/2024/04/16/202404161756155.png)
...
![image-20240416175539688](https://s3.yaourt.cn/img/windows/2024/04/16/202404161755833.png)
...
2. 根据4.2.3章节获取的相关参数填入到配置项
   ...
   ![image-20240416180050496](https://s3.yaourt.cn/img/windows/2024/04/16/202404161800662.png)
